...
#SBATCH -J GLnexus_Brahman
#SBATCH -t 04:00:00
#SBATCH -n 1
#SBATCH -c 12
#SBATCH --mem=120G
#SBATCH -o logs/glnexus_brahman_%j.out
#SBATCH -e logs/glnexus_brahman_%j.err
...
OUTDIR="$BREEDMAPS/louis/results/glnexus/bonsmara/UOA_Brahman_1"
mkdir -p ${OUTDIR}

OUT_PREFIX="cohort_8Bons_Brahman"

GVCFS=""
for ID in 033 034 035 036 037 038 039 040; do
    GVCF=$(ls $BREEDMAPS/louis/results/DeepVariant/UOA_Brahman_1/pr_219_${ID}/*.clean.g.vcf.gz)
    GVCFS="${GVCFS} ${GVCF}"
    echo "Trouvé : ${GVCF}"
done
