...
#SBATCH -J GLnexus_Afri
#SBATCH -t 04:00:00
#SBATCH -n 1
#SBATCH -c 8
#SBATCH --mem=120G
#SBATCH -o logs/glnexus_afrikaner_%j.out
#SBATCH -e logs/glnexus_afrikaner_%j.err
...
export APPTAINERENV_MALLOC_ARENA_MAX=2
export SINGULARITYENV_MALLOC_ARENA_MAX=2

OUTDIR="$BREEDMAPS/louis/results/glnexus/Afrikaner/ARS_UCD2.0"
mkdir -p ${OUTDIR}

OUT_PREFIX="cohort_8Afrikaner_2.0"

GVCFS=""
for ID in 025 026 027 028 029 030 031 032; do
    GVCF=$(ls $BREEDMAPS/louis/results/DeepVariant/Afrikaner_ARS2.0/pr_219_${ID}/*.clean.g.vcf.gz)
    GVCFS="${GVCFS} ${GVCF}"
    echo "Trouvé : ${GVCF}"
done
